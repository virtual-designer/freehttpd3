...
int
xpoll_register_fd (struct xpoll *xp, int fd, xevent_type_t events, xevent_opt_t opts)
{
#if PLATFORM_LINUX
    struct epoll_event ev;
    
    ev.events = events | (opts & XPOLL_ET ? EPOLLET : 0) | (opts & XPOLL_ONESHOT ? EPOLLONESHOT : 0);
    ev.data.fd = fd;
...
int
xpoll_modify_registered_fd (struct xpoll *xp, int fd, xevent_type_t events, xevent_opt_t opts)
{
#if PLATFORM_LINUX
    struct epoll_event ev;
...
int
xpoll_unregister_fd (struct xpoll *xp, int fd, xevent_type_t events, xevent_opt_t opts)
{
#if PLATFORM_LINUX
...
{
#if PLATFORM_LINUX
    return epoll_wait (xp->fd, (struct epoll_event *) events, max_events, timeout);
#elif PLATFORM_BSD
