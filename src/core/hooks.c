...
{
	for (size_t i = 0; i < __FH_HOOK_TYPE_MAX; i++)
	{
		struct fh_hook_cb *current = list->heads[i];
...
static inline bool
fh_hook_list_add_entry (struct fh_hook_cb **head, struct fh_hook_cb **tail,
						uint32_t module_id, fh_hook_cb_generic_t cb_ptr)
{
...
bool
fh_hook_add (struct fh_hook_list *list, uint32_t module_id,
